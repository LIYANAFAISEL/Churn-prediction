...
export class ChurnComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      ...
      tenure:            [1, [Validators.required, Validators.min(0), Validators.max(72)]],
  ...
  // Pre-fill with a high-risk customer so recruiters can test easily
  ...
  // Pre-fill with a low-risk customer
  prefillLowRisk(): void {
    this.form.patchValue({
      gender: 'Female',
      SeniorCitizen: 0,
  ...
  onSubmit(): void {
    ...
    this.churnService.predict(this.form.value).subscribe({
      ...
      error: (err) => {
        this.error = 'Prediction failed. Make sure the API is running.';
        this.loading = false;
        console.error(err);
      }
    });
